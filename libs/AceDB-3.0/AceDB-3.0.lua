...
function DBObjectLib:RegisterDefaults(defaults)
	if defaults and type(defaults) ~= 'table' then
		error(
			("Usage: AceDBObject:RegisterDefaults(defaults): 'defaults' - table or nil expected, got %q."):format(type(defaults)),
			2
		)
	end
...
function DBObjectLib:RegisterNamespace(name, defaults)
	...
	if self.children and self.children[name] then
		error(
			("Usage: AceDBObject:RegisterNamespace(name, defaults): 'name' - a namespace called %q already exists."):format(name),
			2
		)
	end
